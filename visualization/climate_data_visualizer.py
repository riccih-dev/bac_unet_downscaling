...
from preprocessor.utility import sort_ds
import warnings
...
class ClimateDataVisualizer:
    ...
    def plot_climate_data(self, era5, era5_lsm_z, cerra, cerra_lsm_orog):
        ...
        fig_era5, ax_era5 = plt.subplots(nrows=1, ncols=3, figsize=(18,14),
                                         subplot_kw={"projection": ccrs.PlateCarree()},
                                         gridspec_kw={'width_ratios': [1, 1, 1]})

        self.__plot_t2m(ax_era5[0], era5['longitude'], era5['latitude'], era5['t2m'], 'ERA5 2m Temperature')
        self.__plot_lsm(ax_era5[1], era5_lsm_z['longitude'], era5_lsm_z['latitude'], era5_lsm_z, 'ERA5 Land-Sea Mask')
        self.__plot_orog(ax_era5[2], era5_lsm_z['longitude'], era5_lsm_z['latitude'], era5_lsm_z, 'ERA5 Orography', 'z')
        ...
        self.__plot_t2m(ax_cerra[0], cerra['longitude'], cerra['latitude'], cerra['t2m'], 'CERRA 2m Temperature')
        self.__plot_lsm(ax_cerra[1], cerra_lsm_orog['longitude'], cerra_lsm_orog['latitude'], cerra_lsm_orog, 'CERRA Land-Sea Mask')
        self.__plot_orog(ax_cerra[2], cerra_lsm_orog['longitude'], cerra_lsm_orog['latitude'], cerra_lsm_orog, 'CERRA Orography')

        plt.show()

    def __set_plot(self, ax, map_, title, data, invert_color=False):
        '''
        ...
        - data (xarray.Dataset): Climate data.
        ...
        lat_ticks = np.arange(data['latitude'].min(), data['latitude'].max(), 5)
        lon_ticks = np.arange(data['longitude'].min(), data['longitude'].max(), 5)
